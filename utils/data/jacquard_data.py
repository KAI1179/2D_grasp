...
class JacquardDataset(GraspDatasetBase):
    ...
    def __init__(self, file_path, ds_rotate=0, **kwargs):
        ...
        self.mask_files = [f.replace('RGB.png', 'mask.png') for f in self.rgb_files]  ##mj 返回所有mask.png文件的路径列表
    ...
    def get_mask(self, idx, rot=0, zoom=1.0, normalise=True): ## mj33333
        mask_img = image.Image.from_file(self.mask_files[idx])
        mask_img.rotate(rot)
        mask_img.zoom(zoom)
        mask_img.resize((self.output_size, self.output_size)) #从（1024.1024）转换尺寸到（300.300）
        if normalise:
            mask_img.normalise() #归一化
            mask_ndarray = mask_img.img
            #mask_tensor = torch.from_numpy(mask_ndarray)
        return mask_ndarray
